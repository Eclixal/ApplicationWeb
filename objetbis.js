class CalculDistance {
    ...
    calculDistanceTrajet(activite) {
        ...
        for (let i = 0 ; i < activite.length-1; i++) {
            distance = distance + this.calculDistance2PointsGPS(activite[i].latitude, activite[i].longitude, activite[i+1].latitude, activite[i+1].longitude);
        }
        return distance;
    }
